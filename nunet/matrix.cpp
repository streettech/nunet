...
/* VECTOR_ROW PUBLIC FUNCTIONS 
*********************************/

VectorRow& VectorRow::operator+=(const VectorRow& v)
{
...
VectorRow& VectorRow::operator*=(double num) {
    for (int i = 0; i < m.rows(); ++i) {
        for (int j = 0; j < m.cols(); ++j) {
            p[i][j] *= num;
        }
...
Matrix operator+(Matrix m, VectorRow v) {
    if (m.cols() != v.cols())
        return m;
    ...
    Matrix temp(res, m.rows(), m.cols());
    return temp;
}

VectorRow operator+(VectorRow v1, VectorRow v2) {
    VectorRow temp(v1);
    return (temp += v2);
}
...
ostream& operator<<(ostream& os, const VectorRow& v)
{
    for (int i = 0; i < v.rows_; ++i) {
        os << v.p[i][0];
        for (int j = 1; j < v.cols_; ++j) {
            os << " " << v.p[i][j];
        }
        os << endl;
    }
...
istream& operator>>(istream& is, VectorRow& v)
{
    for (int i = 0; i < v.rows_; ++i) {
